...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('pacientes.index') }}" class="inline-flex items-center justify-center w-10 h-10 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white border border-slate-700 rounded-xl transition-all">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
        </a>
        <div>
            <h1 class="text-3xl font-black text-white tracking-tight">Editar <span class="text-teal-400">Paciente</span></h1>
            <p class="text-slate-400 text-sm mt-1 font-medium">Modifica los datos del registro existente</p>
        </div>
    </div>

    {{-- FORMULARIO --}}
    <form action="{{ route('pacientes.update', $paciente) }}" method="POST" class="bg-slate-800/50 border border-slate-700 shadow-2xl rounded-2xl p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text"
                    ...
                    title="Ingrese una identificación válida entre 6 y 11 dígitos."
                    class="w-full bg-slate-900 border border-slate-700 text-slate-100 text-sm rounded-xl px-4 py-3.5 focus:border-teal-400 focus:ring-4 focus:ring-teal-400/10 transition-all outline-none">
            </div>
            ...
            <div>
                ...
                <input type="text"
                    ...
                    pattern="3[0-9]{9}"
                    oninput="this.value=this.value.replace(/[^0-9]/g,'')"
                    title="Ingrese un celular colombiano válido."
                    class="w-full bg-slate-900 border border-slate-700 text-slate-100 text-sm rounded-xl px-4 py-3.5 focus:border-teal-400 focus:ring-4 focus:ring-teal-400/10 transition-all outline-none">
            </div>
